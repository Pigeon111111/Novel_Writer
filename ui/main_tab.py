...
def build_left_layout(self):
    ...
    # 导入并添加自动化按钮
    from ui.automation_buttons import add_automation_buttons
    add_automation_buttons(self, self.left_frame, row=3)

    # 调整日志文本框的行号
    self.left_frame.grid_rowconfigure(4, weight=0)  # 日志标签
    self.left_frame.grid_rowconfigure(5, weight=1)  # 日志文本框
