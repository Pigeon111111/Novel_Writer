...
class OpenAIEmbeddingAdapter(BaseEmbeddingAdapter):
    ...
    def __init__(self, api_key: str, base_url: str, model_name: str):
        # 如果API Key为空，使用默认值避免报错
        safe_api_key = api_key if api_key and api_key.strip() else "sk-no-key-required"
...
class SiliconFlowEmbeddingAdapter(BaseEmbeddingAdapter):
    ...
    def __init__(self, api_key: str, base_url: str, model_name: str):
        ...
        # 如果API Key为空，使用默认值
        safe_api_key = api_key if api_key and api_key.strip() else "siliconflow-no-key"
        self.headers = {
            "Authorization": "Bearer {api_key}".format(api_key=safe_api_key),
            "Content-Type": "application/json"
        }
...
class OllamaEmbeddingAdapter(BaseEmbeddingAdapter):
    """
    适配本地部署的 FastAPI Embedding 服务的适配器
    对应你部署在局域网的 BGE-M3 模型服务
    """
    def __init__(self, model_name: str, base_url: str):
        """
        :param base_url: 部署服务的基础地址，例如 http://localhost:5000
        :param model_name: 模型名称（仅用于标识，实际由部署服务决定）
        """
        self.base_url = base_url.rstrip("/")
        ...
        logging.info(f"初始化 Ollama Embedding 适配器: {self.embeddings_url}")
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            logging.debug(f"Sending embedding request to {self.embeddings_url} with {len(texts)} texts")
            ...
            if not embeddings or len(embeddings) != len(texts):
                logging.error(f"Embedding count mismatch: expected {len(texts)}, got {len(embeddings) if embeddings else 0}")
                return [[] for _ in range(len(texts))]
            
            for i, emb in enumerate(embeddings):
                if not emb or len(emb) == 0:
                    logging.warning(f"Empty embedding at index {i}")
                    
            logging.debug(f"Successfully got {len(embeddings)} embeddings")
            return embeddings
            
        except requests.exceptions.Timeout:
            logging.error(f"Embedding API request timed out after 120s")
            return [[] for _ in range(len(texts))]
        except requests.exceptions.RequestException as e:
            logging.error(f"Embedding API request failed: {str(e)}")
            return [[] for _ in range(len(texts))]
        except (KeyError, ValueError, TypeError) as e:
            logging.error(f"Error parsing Embedding API response: {str(e)}")
            return [[] for _ in range(len(texts))]
    ...
    def embed_query(self, query: str) -> List[float]:
        ...
        if embeddings and embeddings[0]:
            return embeddings[0]
        logging.warning(f"Empty embedding returned for query: {query[:50]}...")
        return []

class LocalEmbeddingAdapter(BaseEmbeddingAdapter):
    """
    适配本地部署的通用Embedding服务（OpenAI兼容接口）
    支持Ollama、LocalAI、vLLM等提供OpenAI兼容接口的服务
    """
    def __init__(self, api_key: str, base_url: str, model_name: str):
        """
        :param api_key: API密钥（本地服务通常不需要，可留空或使用任意值）
        :param base_url: 本地服务地址，例如 http://localhost:8000/v1
        ...
        self.headers = {
            "Authorization": f"Bearer {api_key if api_key else 'local-no-key'}",
            "Content-Type": "application/json"
        }
        self.model_name = model_name
        logging.info(f"初始化本地Embedding适配器: {self.url}, 模型: {self.model_name}")
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            result = response.json()
            if "data" not in result:
                logging.error(f"Invalid response format from Local Embedding API: {result}")
                return [[]] * len(texts)
            return [item.get("embedding", []) for item in result["data"]]
        except requests.exceptions.RequestException as e:
            logging.error(f"Local Embedding API request failed: {str(e)}")
            return [[]] * len(texts)
        except (KeyError, IndexError, ValueError, TypeError) as e:
            logging.error(f"Error parsing Local Embedding API response: {str(e)}")
            return [[]] * len(texts)
...
def create_embedding_adapter(
    ...
    if fmt == "openai":
        ...
        return LocalEmbeddingAdapter(api_key, base_url, model_name)
    else:
