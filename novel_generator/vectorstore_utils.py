    ...
    try:
        class LCEmbeddingWrapper(LCEmbeddings):
            ...
            def embed_documents(self, texts):
                try:
                    result = call_with_retry(
                        func=self.adapter.embed_documents,
                        max_retries=3,
                        texts=texts
                    )
                    if not result or (isinstance(result, list) and len(result) == 0):
                        logging.warning("Embedding documents returned empty result")
                        return [[0.0] * 1024 for _ in range(len(texts))]
                    return result
                except Exception as e:
                    logging.error(f"Embedding documents failed: {e}")
            ...
            def embed_query(self, query: str):
                try:
                    res = call_with_retry(
                        func=self.adapter.embed_query,
                        max_retries=3,
                        query=query
                    )
                    if not res or (isinstance(res, list) and len(res) == 0):
                        logging.warning(f"Query embedding returned empty result for: {query[:50]}...")
                        return [0.0] * 1024
                    return res
                except Exception as e:
        ...
        chroma_embedding = LCEmbeddingWrapper(embedding_adapter)
