# AI Novel Generator - 文档目录

## 📚 文档导航

### 快速开始

- [安装指南](installation.md) - 详细的安装步骤和环境配置
- [快速开始](quickstart.md) - 5分钟快速上手教程
- [配置说明](configuration.md) - 完整的配置参数说明

### 使用指南

- [基础使用](basic-usage.md) - 基本功能使用教程
- [高级功能](advanced-features.md) - 高级功能详解
- [自动化管线](automation.md) - 自动化生成流程
- [平台集成](platform-integration.md) - 平台投稿功能

### 开发文档

- [架构设计](architecture.md) - 系统架构和设计理念
- [API文档](api.md) - RESTful API接口文档
- [开发指南](development.md) - 本地开发和调试
- [测试指南](testing.md) - 测试编写和运行

### 参考文档

- [配置参考](config-reference.md) - 完整配置参数列表
- [模型支持](models.md) - 支持的LLM模型列表
- [审计维度](audit-dimensions.md) - 33维度审计系统说明
- [常见问题](faq.md) - 常见问题解答

---

## 🔗 外部资源

- [GitHub仓库](https://github.com/YILING0013/AI_NovelGenerator)
- [更新日志](../CHANGELOG.md)
- [贡献指南](../CONTRIBUTING.md)
- [问题反馈](https://github.com/YILING0013/AI_NovelGenerator/issues)

---

## 📖 文档贡献

发现文档错误或想要改进文档？欢迎提交Pull Request！

文档使用Markdown格式编写，位于 `docs/` 目录。

---

## 📞 获取帮助

- 💬 [GitHub Discussions](https://github.com/YILING0013/AI_NovelGenerator/discussions)
- 🐛 [问题反馈](https://github.com/YILING0013/AI_NovelGenerator/issues)
- 📧 [邮件联系](mailto:your-email@example.com)
