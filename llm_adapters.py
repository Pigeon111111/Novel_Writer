...
class DeepSeekAdapter(BaseLLMAdapter):
    ...
    def __init__(self, api_key: str, base_url: str, model_name: str, max_tokens: int, temperature: float = 0.7, timeout: Optional[int] = 600):
        ...
        self.api_key = api_key if api_key and api_key.strip() else "sk-no-key-required"
...
class OllamaAdapter(BaseLLMAdapter):
    ...
    def __init__(self, api_key: str, base_url: str, model_name: str, max_tokens: int, temperature: float = 0.7, timeout: Optional[int] = 600):
        self.base_url = check_base_url(base_url)
        # Ollama通常不需要API Key，使用默认值
        self.api_key = api_key if api_key and api_key.strip() else 'ollama'
        self.model_name = model_name
        self.max_tokens = max_tokens
        self.temperature = temperature
        self.timeout = timeout
...
class MLStudioAdapter(BaseLLMAdapter):
    def __init__(self, api_key: str, base_url: str, model_name: str, max_tokens: int, temperature: float = 0.7, timeout: Optional[int] = 600):
        self.base_url = check_base_url(base_url)
        # ML Studio通常不需要API Key，使用默认值
        self.api_key = api_key if api_key and api_key.strip() else "ml-studio-no-key"
        self.model_name = model_name
        self.max_tokens = max_tokens
        self.temperature = temperature
        self.timeout = timeout
...
class LocalLLMAdapter(BaseLLMAdapter):
    """
    适配本地部署的LLM服务（如Ollama、vLLM、LocalAI等）
    支持OpenAI兼容接口，无需API Key即可使用
    """
    def __init__(self, api_key: str, base_url: str, model_name: str, max_tokens: int, temperature: float = 0.7, timeout: Optional[int] = 600):
        self.base_url = check_base_url(base_url)
        # 本地模型不需要API Key，使用默认值
        self.api_key = api_key if api_key and api_key.strip() else "local-no-key-required"
        self.model_name = model_name
        self.max_tokens = max_tokens
        self.temperature = temperature
        self.timeout = timeout

        logging.info(f"初始化本地LLM适配器: {self.base_url}, 模型: {self.model_name}")
    ...
    def invoke(self, prompt: str) -> str:
        try:
            response = self._client.invoke(prompt)
            if not response:
                logging.warning("No response from LocalLLMAdapter.")
                return ""
            return response.content
        except Exception as e:
            logging.error(f"本地LLM调用失败: {e}")
            return ""
...
def create_llm_adapter(
    ...
    if fmt == "deepseek":
        ...
        return LocalLLMAdapter(api_key, base_url, model_name, max_tokens, temperature, timeout)
    else:
